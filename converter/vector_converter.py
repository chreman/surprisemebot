...
def main(args):
    ...
    dois = [r['doi'] for r in raw_df.select('doi').collect()]
    mat = np.row_stack([r
                        for r in raw_df.select('topicDistribution').rdd.toLocalIterator()])
    # df = raw_df.toPandas()
    # with open(args.output+"/df.pkl") as outfile:
    #     pickle.dump(df, outfile)
    # words = df['word'].tolist()
    # mat = np.row_stack(df['vector'].tolist())
    with open(args.output+"/dois.pkl", "wb") as outfile:
        pickle.dump(dois, outfile)
