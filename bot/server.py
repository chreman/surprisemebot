...
class SurprisemeBot(Daemon):
    """docstring for EmbedBot"""
    def __init__(self, pidfile):
        super(SurprisemeBot, self).__init__(pidfile)
        logname = ("SurprisemeBot.log")
        self.log = logging.getLogger("surprisemebot")
    ...
    def start_engine(self):
        self.similarityEngine = SimilarityEngine(self.modelpath)
    ...
    def handle_mentions(self):
        for mention in self.state['mention_queue']:
            ...
            if len(not_in_vocab) != 0:
                ...
                reply = prefix+" Sorry, "+", ".join(not_in_vocab)+" not in my data set."
    ...
    def formulate_reply(self, prefix, text, query, signs):
        if len(query) == 1:
            result = self.similarityEngine.get_related_papers(query[0], 5)
            result = ", ".join(self.linkify(result))
            reply = "(Somehow) related papers to " + query[0] + ": " + result
        else:
            result = self.w2vengine.get_mixture_of_papers(query, signs, 5)
            result = ", ".join(self.linkify(result))
            reply = "(Somehow) related papers to:" + result
        return " ".join([prefix, reply])

    def linkify(self, result):
        result = ["http://dx.doi.org/"+r for r in result]
        return result

    def check_dataset(self, query):
        # expects list of dois
        q = set(query)
        diff = q.difference(set(self.similarityEngine.vocabulary))
        return list(diff)
...
if __name__ == '__main__':
    bot = SurprisemeBot('surprisemebot.pid')
